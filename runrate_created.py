...
def bucketize_issues_by_interval(issues, mode, interval="weeks"):
    """
    ...
    print(f"\n🔍 Processing issues for {interval} bucketization...")
    print(f"Mode: {mode}")
...
if "created.rate" in yaml_file.lower():
    print(f"created.rate mode detected based on filename: {yaml_file}.")
    # You can set a flag or handle import-specific logic here if needed
    output_file = basename + "." + tablename + timestamp + ".created_rate.jira.csv"
    mode = "open_closed"
else:
...
'''for i, (bucket, (year, week_num, start_date, end_date)) in enumerate(zip(weekly_buckets_resolved, week_info_resolved)):
    ...
    # only print the counts, week header was already printed above
    #coord = f"{get_column_letter(col + 1 + i + 1)}{row + 2}"
