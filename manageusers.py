...
def read_csv_files(folder_path: Path) -> list:
    ...
    print(f"[read_csv_files] ✓ Completed. Total documents: {len(documents)}")
    return documents
...
def retrieve():
    """
    Main MCP retrieval endpoint - returns all CSV files from user's vectorstore folder
    Expects JSON: {"query": "search query", "max_results": 5}
    Query parameter is optional and ignored - all documents are always returned
    max_results: maximum number of CSV files to return (default: None = all files)
    """
    try:
        ...
        documents = read_csv_files(user_folder)
...
def list_documents():
    """
    List all CSV files in the user's vectorstore folder
    """
    try:
        api_key = request.api_key
        ...
        print(f"[/mcp/v1/documents GET] ✓ Returning {len(documents)} documents")
        return jsonify(response)
